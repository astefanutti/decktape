...
parser.script('decktape').options({
    ...
    timeout: {
        ...
        metavar: '<ms>',
        default: 20000,
        help: 'Duration in milliseconds to use when waiting for the slide deck to load initially',
    },
    screenshots: {
        default: false,
        flag: true,
...
(async() => {
    ...
    page
        .on('console', async msg => {
            const args = msg.args().length ?
                await Promise.all(msg.args().map(arg => arg.executionContext().evaluate(obj => obj, arg))) : [msg.text()];
    ...
    const load = page.waitForNavigation({ waitUntil: 'load', timeout: options.timeout });
